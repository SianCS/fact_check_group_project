...
function Badge({ label }: { label?: string }) {
  ...
  return (
    <span className={`inline-block px-2 py-0.5 rounded-full text-xs ring-1 ${cls}`}>
      {label ?? "-"}
    </span>
  );
}
...
export default function Page() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((c) =>
      (c.claimReview ?? []).some((r) =>
        (r.textualRating || "").toLowerCase().includes(ratingFilter.toLowerCase())
      )
    );
  }, [items, ratingFilter]);
  ...
  async function fetchClaims(opts: { append?: boolean; pageToken?: string }) {
    ...
    try {
      ...
      setItems((prev) => (append ? [...prev, ...(data.claims ?? [])] : data.claims ?? []));
    } catch (err: unknown) {
    ...
    <main className="min-h-dvh bg-gradient-to-b from-slate-50 via-white to-slate-50 text-gray-900">
      ...
      <section className="max-w-5xl mx-auto px-4 pt-6">
        <div className="rounded-2xl border bg-white/80 backdrop-blur p-4 md:p-5 shadow-sm">
          <form ref={formRef} onSubmit={onSearch} className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 rounded-xl border outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div className="flex gap-2 items-center">
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                  setLang(e.target.value as "th" | "en")
                }
                title="Language"
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 justify-between">
            <div className="flex gap-2">
              <Pill active={ratingFilter === ""} onClick={() => setRatingFilter("")}>
                ทั้งหมด
              </Pill>
              <Pill active={ratingFilter === "false"} onClick={() => setRatingFilter("false")}>
                False
              </Pill>
              <Pill active={ratingFilter === "misleading"} onClick={() => setRatingFilter("misleading")}>
                Misleading
              </Pill>
              <Pill active={ratingFilter === "true"} onClick={() => setRatingFilter("true")}>
                True
              </Pill>
              <Pill active={ratingFilter === "correct"} onClick={() => setRatingFilter("correct")}>
                Correct
              </Pill>
            </div>
      ...
      <section className="max-w-5xl mx-auto px-4 py-6">
        ...
        {!loading && items.length === 0 && !error && (
          <div className="rounded-2xl border border-dashed bg-white p-6 text-gray-600">
            เริ่มค้นหาได้เลย — ลองคำกว้าง ๆ เช่น <b>วัคซีน</b> หรือ <b>covid</b> จากนั้นค่อยใช้ฟิลเตอร์ผลตรวจ
          </div>
        )}
        ...
        {loading && (
          <ul className="space-y-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <li key={i} className="animate-pulse rounded-2xl border bg-white p-5">
            ...
            {filtered.map((c, idx) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    <div className="text-xs text-gray-500">
                      {c.claimDate ? new Date(c.claimDate).toLocaleDateString() : ""}
                    </div>
                    <h2 className="font-semibold text-lg">{c.text || "(ไม่มีข้อความอ้างอิง)"}</h2>
                ...
                {(c.claimReview ?? []).map((r, i) => (
                  <div key={i} className="mt-4 rounded-xl border p-4 bg-gray-50">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      <div className="text-sm">
                        ผู้ตรวจสอบ: <b>{r.publisher?.name || r.publisher?.site || "-"}</b>
                      </div>
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      วันที่รีวิว: {r.reviewDate ? new Date(r.reviewDate).toLocaleDateString() : "-"} · ภาษาบทรีวิว:{" "}
                      {r.languageCode || lang}
                    </div>
      ...
      <footer className="max-w-5xl mx-auto px-4 py-8 text-center text-xs text-gray-500">
        ข้อมูลจากผู้ตรวจสอบข้อเท็จจริง (Fact-Checking Publishers) ผ่าน Google Fact Check Tools API
      </footer>
    </main>
  );
}
