...
export default function SafeBrowsingPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const r = await fetch(`/api/httpcheck?url=${encodeURIComponent(target)}`, {
        cache: "no-store",
      });
  ...
  const examples = [
    { label: "Phishing (ทดสอบ)", href: "https://testsafebrowsing.appspot.com/s/phishing.html" },
    { label: "Malware (ทดสอบ)", href: "https://testsafebrowsing.appspot.com/s/malware.html" },
    { label: "Unwanted (ทดสอบ)", href: "https://testsafebrowsing.appspot.com/s/unwanted.html" },
  ];
    ...
    <main className="min-h-dvh bg-gradient-to-b from-slate-50 via-white to-slate-50 text-gray-900">
      ...
      <header className="sticky top-0 z-20 border-b bg-white/70 backdrop-blur">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-xl bg-emerald-600 grid place-items-center text-white">🛡️</div>
            <h1 className="text-lg md:text-xl font-bold">URL Safety Check</h1>
          </div>
          <Link href="/" className="text-sm rounded-full px-3 py-1.5 bg-blue-600 text-white hover:bg-blue-700 transition">
            ← กลับหน้าแรก
          </Link>
        </div>
      </header>
      ...
      <section className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="rounded-2xl border bg-white/80 backdrop-blur p-4 md:p-5 shadow-sm">
          <form ref={formRef} onSubmit={onSubmit} className="flex flex-col md:flex-row gap-3">
        ...
        {!loading && result && (
          <div
            className={`mt-4 rounded-2xl p-5 border ${
              isUnsafe ? "border-red-300 bg-red-50" : "border-emerald-300 bg-emerald-50"
            }`}
          >
              ...
              <div>
                ...
                <div className="mt-1 text-2xl font-semibold">
                  ...
                  {isUnsafe ? (
                    ...
                    <span className="text-emerald-700">ปลอดภัย (ไม่พบรายการ)</span>
                  )}
                </div>
              </div>
            ...
            {isUnsafe ? (
                ...
                {result!.map((m, i) => (
                  <li key={i} className="rounded-xl border bg-white p-3">
                    <div className="text-sm">
                      ...
                      <b>Platform:</b> {m.platformType || "ANY_PLATFORM"}
                    </div>
                    <div className="text-sm break-all">
                      <b>URL:</b> {m.threat?.url || "-"}
                    </div>
                    {m.cacheDuration && (
                      <div className="text-xs text-gray-500">Cache: {m.cacheDuration}</div>
                    )}
                  </li>
                ))}
              </ul>
              ...
              <p className="mt-3 text-sm text-gray-700">
                หมายเหตุ: “ไม่พบ” = ลิงก์ยังไม่อยู่ในลิสต์ของ Google Safe Browsing ณ ตอนนี้
                (ไม่ได้การันตีความปลอดภัย 100% เสมอไป)
              </p>
            )}
          </div>
        )}
        ...
        {!loading && !result && !error && (
          <div className="mt-4 rounded-2xl border border-dashed bg-white p-6 text-gray-600">
            วาง URL เพื่อเริ่มตรวจ—ระบบจะเทียบกับลิสต์ฟิชชิ่ง/มัลแวร์ของ Google แบบเรียลไทม์
          </div>
        )}
      </section>
